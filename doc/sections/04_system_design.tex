...
Based on the requirements analysis and literature review, YOLO11 was taken as
the initial design candidate for the object detector, to be fine-tuned for
luggage and people detection in surveillance footage. YOLO11 has demonstrated
strong performance on small object detection in the work by
\citet{abandoned2025vrsalovic}, making it a suitable starting point for the
target application. Ease of optimisation for edge deployment on NVIDIA hardware
is also a critical factor in this choice, with YOLO models being widely
supported by TensorRT optimisations (see~\ref{sec:tensorrt_optimisations}). The
final selected architecture was determined during implementation
(section~\ref{sec:model_architecture_selection}).
...
\begin{itemize}
	...
	\item \textbf{Tracker (\texttt{nvtracker}):} Maintains object identities across
	      frames using NvDeepSORT (DeepStream's DeepSORT implementation) with a
	      ResNet-50 ReID model for appearance-based association.
