...
\begin{itemize}
	\item \textbf{Traditional Methods:}
	      ...
	      highlights the main issue of these methods being their high requirements for
	      computational power.
	...
	\item \textbf{Modern Methods:}
	      ...
	      transformers (DETRs), has introduced a new paradigm in object detection.
...
ABODA dataset which is publicly available on GitHub
\url{https://github.com/kevinlin311tw/ABODA} \citep{lin2015aboda}. These
datasets provide a common ground for researchers to evaluate and compare their
results in a niche field. Due to the limited availability, and age, of these
datasets, there is a clear gap in the field for modern, publicly available
datasets for abandoned object detection.
...
\begin{table}[!htbp]
	...
	\begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
		...
		\addlinespace
		Mean Average Precision ($\mathrm{mAP}$)                                                                                         & The mean of AP across all classes.                                                                            & Higher mAP indicates better overall performance across all classes.                                                                                                                    \\
		\addlinespace
		...
		\addlinespace
		Cohen's Kappa ($\kappa$)                                                                                                        & A measure of accuracy that takes into account the probability of a random agreement.                          & 1 indicating perfect agreement, 0 equivalent to chance, and -1 indicating complete disagreement. It has the same use-cases as $\mathrm{MCC}$.                                          \\
		\addlinespace
...
\begin{table}[!htbp]
	...
	\begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
		...
		\addlinespace
		Objectness Score                      & As defined in the original YOLO paper \citep{redmon2016lookonceunifiedrealtime}, for probability of object presence within a bounding box $P(\mathrm{Obj}) \in [0, 1]$: $$\mathrm{Objectness} = P(\mathrm{Obj}) \cdot \mathrm{IoU} \in [0,1]$$ \\
		\addlinespace
...
generated as an aggregate of $C$ predictions of conditional class probabilities
...
The latest versions, YOLOv8, YOLO11, and YOLO26, have built upon this
foundation. The fundamental principles of unified detection and grid-based
prediction remain, but with significant improvements. The survey by
\citet{kotthapalli2025yolov1yolov11comprehensivesurvey} describes the taxonomy
of YOLO models' improvements up to YOLO11, highlighting five key areas:

\begin{itemize}
	\item \textbf{Backbone networks:} the evolution of backbone architectures
	      from Darknet-19 in YOLOv2, to Darknet-53 in YOLOv3, to CSPDarknet53 in
	      YOLOv4/v5, and the more recent CSP-derived blocks in YOLOv8 (the
	      \texttt{C2f} module) and YOLO11 (the \texttt{C3k2} module) has led to
	      significant improvements in feature extraction capabilities at
	      comparable or reduced parameter counts.
	\item \textbf{Neck:} progression from the single $S\times S$ grid in
	      YOLOv1, through the feature pyramid network (FPN) first incorporated
	      in YOLOv3, to the PAN (Path Aggregation Network) necks adopted in
	      YOLOv4 and carried forward through YOLOv5, YOLOv8, and YOLO11 ---
	      with YOLO11 additionally inserting a \texttt{C2PSA} spatial-attention
	      block after SPPF to refine feature maps before the neck.
...
YOLO26, released after the initial literature review for this project, extends
these trends with an architecture redesigned for edge deployment. Ultralytics
claim significant improvements to the speed-accuracy Pareto front across all
model sizes compared to YOLO11 \citep{jocher2025yolo26, vina2026yolo26}.
However, at the time of writing, no independent peer-reviewed evaluations of
YOLO26 exist in the literature, meaning these claims rely solely on the
developer's own benchmarks. YOLO26 is included in this project's model
evaluation (section~\ref{sec:training_process}) to provide an independent
assessment of its performance in the target domain.

\begin{table}[!htbp]
	\centering
	\small
	\begin{tabularx}{\textwidth}
		{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}m{0.25\columnwidth}
		...
		across tasks.                                                                \\
		\addlinespace
		\textbf{YOLO26}  & Redesigned architecture built for edge and low-power
		devices; improved speed-accuracy Pareto front over YOLO11 across all model
		sizes \citep{jocher2025yolo26}.                                              \\
		\bottomrule
	\end{tabularx}
...
\label{sec:video_analysis_spatio_temporal_modeling_and_object_tracking}
...
\label{sec:real_time_processing_on_edge_devices}
